...
- 포커스 소유권: 공통 foundation의 전역 표시를 기본으로 하고, 가시성 보완·조작 범위 표현에 근거가 있는 자체 표시를 예외로 허용한다 (`decision-log.md` D-010). foundation·preset 사전 검증과 기존 표시의 검증은 후속 단계이며, §3-1의 light 대비 결함도 남아 있다.
